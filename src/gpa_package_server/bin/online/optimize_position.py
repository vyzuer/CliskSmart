...
def get_results(graph, image, img_src, dump_path=None):
    ...
    pos_dumps = None
    if dump_path is not None and _SERVER == False:
    ...
    for node in graph.psuedo_nodes:
        ...
        circle_radius = 1.*node.radius*size_scale
        # opencv its bgr
        ...
        # doing this to flip the screen
        # will only work on my phone, need to fix
        rec_positions.append([int(x_0), int(480-y_0), int(x_1), int(480-y_1)])
...
def gen_pos_rec(s_objects, p_objects, psuedo_objs, img, img_src, visualize=False, dump_path=None, server=False):
    ...
    graph= sgm.create_graph(s_objects, p_objects, psuedo_objs)

    graph = sgm.minimize_energy(graph, img_src, visualize=False, dump_path=dump_path)
