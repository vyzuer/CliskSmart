...
def gen_recommendation(img, img_src, num_people, color_data):
    rec_pos = np.zeros(shape=(num_people, 4), dtype=np.int)

    num_people, rec_pos, color_data = pos_rec(img, img_src, gv.gp_dump_path_, gv.gp_model_path_, visualise=False, server=True)
...
def pos_rec(img, img_src, dump_path, gp_model_path, visualise=True, server=False):
    ...
    if not _SERVER:
        print 'scene prediction time: ', time.time() - t0
        t0 = time.time()
    ...
    if not _SERVER:
        print 'get salient objects: ', time.time() - t0
        t0 = time.time()

    # generate position recommendation
    rec_pos, rec_color = opt_pos.gen_pos_rec(s_objects, p_obj_graph, psuedo_objs, img, img_src, visualize=visualise, dump_path=plot_dumps, server=server)

    if not _SERVER:
        print 'rec time: ', time.time() - t0
