...
class Graph:
    ...
    def printData(self):
        ...
        for node in self.nodes:
            ...
            print 'repulsive force', node.force_coulomb
            print 'attractive force', node.force_harmonic
            print
...
tk.Button(root, text="Stop", command=lambda root=root:stop(root)).pack()

# make it cover the entire screen
w, h = root.winfo_screenwidth()/2, root.winfo_screenheight()
root.overrideredirect(1)
root.geometry("%dx%d+%d+%d" % (w, h, 4, 4))
...
# g.paintGraph()
time_spent = 0.0
while (not b_stop and (not g.calculation_finished or dont_finish_calculating)):
    timer = time.time()
    g.calculateStep()
