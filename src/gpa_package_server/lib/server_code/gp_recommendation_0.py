...
def gen_recommendation(img_src, num_people, color_data):
    ...
    num_people, rec_pos, color_data = pos_rec(img_src, gv.gp_dump_path_, gv.gp_model_path_, visualise=False)

    return num_people, rec_pos, color_data


def pos_rec(img_src, dump_path, gp_model_path, visualise=True):
    ...
    scene_obj = s_id.scene_info(img_src, gp_model_path, visualise=visualise, dump_path=plot_dumps)
    ...
    s_objects, p_objects = scene_obj.get_salient_objects()
    ...
    p_obj_graph = scene_obj.get_pobj_for_graph(num_people, m_position, m_size)

    # generate position recommendation
    rec_pos, rec_color = opt_pos.gen_pos_rec(s_objects, p_obj_graph, img_src, visualize=False, dump_path=plot_dumps)
