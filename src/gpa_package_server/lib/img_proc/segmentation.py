...
class SuperPixelSegmentation:
    def __init__(self, image, seg_type=1, min_dist=0.0002, compactness=12, n_segments=100, sigma=3.0, max_iter=20):
        ...
        self.slic_map, self.segments = slic(self.image, compactness=compactness, n_segments=n_segments, sigma=sigma, max_iter=max_iter)
    ...
    def getSegmentationMap(self):
        """ get the segmentation map after merging similar segments using color similarity

        """

        self.__set_timer("computing mean color")
        self.mean_color_hsv, self.mean_color_rgb = self.__get_hsv_mean_color(self.segments[:,3:6]*self.compactness)
        self.__print_timer("computing mean color")

        self.__set_timer("merging superpixels")
        # self.slic_map = self.__segmentation(self.segments, self.min_dist)
        self.__print_timer("merging superpixel")

        self.__set_timer("connected components")
        dummy_map = measure.label(self.slic_map, connectivity=1, background=-1)
        # rename id 0 with something else to avoid further issues
        # dummy_map = self.rename_zero(self.slic_map)
        dummy_map += 1
        # to use the mean color from slic find the segment mapping
        self.id_mapping = self.__find_id_mapping(dummy_map)
    ...
    def rename_zero(self, s_map):
        n_id = np.max(s_map) + 1

        dummy_map = s_map.copy()

        dummy_map[s_map[:]==0] = n_id

        return dummy_map

    def __get_hsv_mean_color(self, lab_segments):
        lab_mean = np.reshape(lab_segments, (1,lab_segments.shape[0],3))
        
        rgb_mean = color.lab2rgb(lab_mean)
        hsv_mean = color.rgb2hsv(rgb_mean)

        # plt.imshow(np.reshape(hsv_mean, (6,8,3)))
        # plt.show()
        # plt.close()
        ...
        return hsv_mean[0], rgb_mean[0]


    def get_slic_segments_color(self):
        return self.segments

    def get_segments_color(self):
        n_segments = np.max(self.smap) + 1

        segments_color = np.zeros(shape=(n_segments, 3))
        for i in range(1, n_segments):

            slic_id = self.get_slic_id(i)
            segments_color[i] = self.segments[slic_id,3:6]


        return segments_color

    def get_rgb_mean_color(self, smap_id):
        slic_id = self.get_slic_id(smap_id)

        return self.mean_color_rgb[slic_id]

    def get_mean_color(self, smap_id):
        slic_id = self.get_slic_id(smap_id)

        return self.mean_color_hsv[slic_id]


    def get_slic_id(self, smap_id):
        return self.id_mapping[self.inv[smap_id]]


    def __find_id_mapping(self, dummy_map):
        num_segments = np.max(dummy_map) + 1
        id_mapping = np.zeros(num_segments, dtype=int)

        height, width = dummy_map.shape

        for i in range(height):
            for j in range(width):
                id_mapping[dummy_map[i,j]] = self.slic_map[i,j]
    ...
    def __find_minimum_pixel_count(self, smap):
        ...
        min_num_pixel = height*width/250
