...
def compute_adj_matrix(Py_ssize_t[:,::1] slic_map):
    cdef Py_ssize_t height, width
    height = slic_map.shape[0]
    width = slic_map.shape[1]

    cdef Py_ssize_t n_segments = np.amax(slic_map) + 1

    cdef Py_ssize_t[:, :] adj_matrix = np.zeros(shape=(n_segments, n_segments), dtype=np.intp)

    # sum features for all segments
    cdef Py_ssize_t x, y, k, ri, bi, kr, kb
...
def compute_mean_color(double[:, :, ::1] image,
                 double[:, ::1] segments, Py_ssize_t[:,::1] slic_map):
    cdef Py_ssize_t height, width
    ...
    return np.asarray(segments), np.asarray(n_segment_elems)
...
def detect_saliency_of_segments(Py_ssize_t[:, :] seg_map, double[:, :] sal_map, double min_saliency):
    ...
    cdef double[:] saliency_list = np.zeros(num_segments, dtype=np.double)
