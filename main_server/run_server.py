...
import online.gp_recommendation as gp_rec
...
def readImage(conn, spath):
    ...
    # resize image
    image, factor = resize(image)
    
    img_src = spath + 'images/' + str(time.time()) + '.jpg'
    # image.save(img_src, "JPEG")
    print 'image saved.'

    print factor

    return image, factor, img_src
...
def receive_data_gp(conn):
    ...
    spath = gv.gp_dump_path_
    image, factor, img_src = readImage(conn, spath)

    return num_people, color_data, image, factor, img_src
...
def receive_data_pa(conn):
    ...
    spath = gv.gp_dump_path_
    image, factor, img_src = readImage(conn, size, spath)
...
def communicate_gp(conn):
    ...
    num_people, color_data, image, factor, img_src = receive_data_gp(conn)

    # generate the recommenations
    try:
        num_people, rec_pos, color_data = gp_rec.gen_recommendation(np.asarray(image), img_src, num_people, color_data)
        # rec_pos = rec_pos/factor
...
def communicate_vp(conn):
    # communicate with the client and exit
    lat, lon, image, factor = receive_data_vp(conn)
...
def start_server():
    ...
    host = "172.18.101.112" # Get local machine name
